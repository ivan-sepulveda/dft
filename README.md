...
- [ ] Translate airport names — city/country names are already translated (locations/countries in translations.py), but airport_name itself is stored in English only, e.g. AGP shows "Malaga-Costa del Sol Airport" on the Spanish site instead of "Aeropuerto de Málaga-Costa del Sol". Needs an airportNames translation dict keyed by iata_code (~250 airports × 4 locales), same pattern as locations/countries
